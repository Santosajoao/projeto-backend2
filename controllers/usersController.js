...
const installSystem = async (req, res) => {
  try {
    ...
    const tickets = [
      { id: 1, name: "Meia", price: 30.0 },
      { id: 2, name: "Inteira", price: 60.0 },
      { id: 3, name: "V.I.P", price: 90.0 },
    ];
    ...
    res
      .status(500)
      .json({ message: "Erro ao realizar a instalação do sistema", error });
  }
};
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (userToDelete.isAdm) {
      return res
        .status(400)
        .json({ message: "Admins não podem ser deletados" });
    }
